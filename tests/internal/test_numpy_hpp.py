...
def test_all_finite():
    ...
    for dtype_ in [float, np.float32, np.float64]:
        ...
        # with inf
        arr = np.ones(100, order="C", dtype=dtype_)
        arr[np.random.randint(0, 99, size=3)] = np.inf
        assert not _mmu_core.all_finite(arr)
...
def test_is_well_behaved_finite_float_t():
    for dtype_ in [float, np.float32, np.float64]:
        ...
        if dtype_ == np.float64:
            ...
            arr[np.random.randint(0, 9, size=3), np.random.randint(0, 9, size=3)] = (
                np.inf
            )
            arr[np.random.randint(0, 9, size=3), np.random.randint(0, 9, size=3)] = (
                np.nan
...
def test_zero_array():
    ...
    for dtype in dtypes:
        ...
        og_arr = np.asarray(np.random.uniform(0, 100, size=100), dtype=dtype)
        arr = og_arr.copy()
        _mmu_core_tests.zero_array(arr)
        if dtype == np.int64:
            assert arr.sum() == 0
...
def test_zero_array_fixed():
    ...
    for dtype in dtypes:
        ...
        og_arr = np.asarray(np.random.uniform(0, 100, size=12), dtype=dtype)
        arr = og_arr.copy()
        _mmu_core_tests.zero_array_fixed(arr)
        if dtype == np.int64:
            assert arr.sum() == 0
