...
def test_confusion_matrix_exceptions():
    ...
    with pytest.raises(RuntimeError):
        mmu_core.confusion_matrix(y, yhat[:100])
    with pytest.raises(RuntimeError):
        mmu_core.confusion_matrix(y[:100], yhat)
    with pytest.raises(RuntimeError):
        mmu_core.confusion_matrix(np.tile(y, 2), yhat)
    with pytest.raises(RuntimeError):
        mmu_core.confusion_matrix(y, np.tile(yhat, 2))
