...
#         )
#
#         conf_mat, metrics = mmu.binary_metrics_proba(y, proba, threshold)
#         assert np.array_equal(conf_mat, sk_conf_mat), (
...
#         conf_mat, metrics = mmu.binary_metrics_proba(y_, proba_, 0.5)
#         assert np.array_equal(conf_mat, sk_conf_mat), (
...
#     # unequal length
#     with pytest.raises(RuntimeError):
#         mmu.binary_metrics_proba(y, proba[:100], 0.5)
#     with pytest.raises(RuntimeError):
#         mmu.binary_metrics_proba(y[:100], proba, 0.5)
#
#     # 2d with more than one row/column for the second dimension or 3d
#     y_shapes = [
#         np.tile(y[:, None], 2),
...
#     for y_, proba_ in itertools.product(y_shapes, proba_shapes):
#         with pytest.raises(RuntimeError):
#             mmu.binary_metrics_proba(y_, proba_, 0.5)
#
#
