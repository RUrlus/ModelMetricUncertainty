...
void confusion_matrix(const size_t n_obs, T* y, T* yhat, int64_t* const conf_mat) {
    for (size_t i = 0; i < n_obs; i++) {
        conf_mat[static_cast<bool>(*y) * 2 + static_cast<bool>(*yhat)] += 1;
        yhat++;
        y++;
    }
}
...
void confusion_matrix(const size_t n_obs, T* y,  T* yhat, int64_t* const conf_mat) {
    for (size_t i = 0; i < n_obs; i++) {
        conf_mat[static_cast<bool>(round(*y)) * 2 + static_cast<bool>(round(*yhat))] += 1;
        yhat++;
        y++;
    }
}
...
void confusion_matrix(
    ...
    for (size_t i = 0; i < n_obs; i++) {
        conf_mat[static_cast<bool>(*y) * 2 + ((*proba) > threshold)] += 1;
        proba++;
        y++;
    }
}
