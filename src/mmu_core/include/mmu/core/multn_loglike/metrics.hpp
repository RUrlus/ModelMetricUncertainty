 ...
 */
#ifndef INCLUDE_MMU_CORE_MULTN_LOGLIKE_METRICS_HPP_
#define INCLUDE_MMU_CORE_MULTN_LOGLIKE_METRICS_HPP_
 ...
 *
 * Which represents the total number of positive predictions and actual positives
 * (excluding TN).
 */
struct PrecisionRecallProfile {
    ...
    /**
     * Compute the metric-specific denominator from confusion matrix.
     * For PR: n3 = FP + FN + TP
     */
    ...
    /**
     * Compute the most conservative probabilities constrained by (precision, recall).
     * This is the loop-optimized version that takes precomputed metric_n and n.
     *
     * @param y        precision value
     * @param x        recall value
     * @param metric_n precomputed n3 = FP + FN + TP
     ...
     * When a confusion matrix element is 0, we can go to the boundary.
     *
    ...
    static inline void get_max_clips(
        ...
        double& max_y_clip,   // precision
        double& max_x_clip) { // recall
        max_y_clip = conf_mat[1] == 0 ? 0.0 : epsilon;  // FP for precision
        max_x_clip = conf_mat[2] == 0 ? 0.0 : epsilon;  // FN for recall
    }
};
...
struct ROCProfile {
    ...
    /**
     * Compute the metric-specific denominator from confusion matrix.
     * For ROC: n2 = FN + TP (actual positives)
     */
    static inline double compute_metric_n(const int64_t* __restrict conf_mat) {
    ...
    /**
     * Compute the most conservative probabilities constrained by (TPR, FPR).
     * This is the loop-optimized version that takes precomputed metric_n and n.
     *
     * @param y        TPR value
     * @param x        FPR value
    ...
    static inline void constrained_fit_safe(
        ...
        const double tpr_inv = y > std::numeric_limits<double>::epsilon()
                                   ? 1.0 / y
                                   : inv_fill;
    ...
    static inline void get_max_clips(
        const int64_t* __restrict conf_mat,
        const double epsilon,
        double& max_y_clip,   // TPR
        double& max_x_clip) { // FPR
        max_y_clip = conf_mat[2] == 0 ? 0.0 : epsilon;  // FN for TPR
        max_x_clip = conf_mat[0] == 0 ? 0.0 : epsilon;  // TN for FPR
    }
};
...
struct RecallPPNProfile {
    ...
    /**
     * Compute the metric-specific denominator from confusion matrix.
     * For Recall-PPN: n_pos = FN + TP (actual positives)
     */
    static inline double compute_metric_n(const int64_t* __restrict conf_mat) {
        return static_cast<double>(conf_mat[2] + conf_mat[3]);
    }
    ...
    static inline void constrained_fit(
        ...
        const double tn = metric_n * y - metric_n * fn_tp_ratio * (metric_n / n);
        const double fp_observed = n - metric_n - (n * y - metric_n);

        // Simplified linear solution (approximation that works well in practice)
        // This follows from the constraint that probabilities must be consistent
    ...
    /**
     * Compute the most conservative probabilities using the full quadratic solution.
     * This is more accurate but slower - use for single-point evaluations.
     *
     * @param y        PPN value
     * @param x        Recall value
     * @param conf_mat confusion matrix [TN, FP, FN, TP]
    ...
    static inline bool constrained_fit_quadratic(
        ...
        const double linear_coef = n_pos * (pp * fn_tp_ratio + y)
                                   + fp * y
                                   + fn_tp_ratio * tn * pp;
        const double discriminant = linear_coef * linear_coef
        ...
        for (double t : {root1, root2}) {
            ...
            double ll = 0.0;
            if (p_tn > 0.0) ll += tn * std::log(p_tn);
            if (p_fp > 0.0) ll += fp * std::log(p_fp);
            if (p_fn > 0.0) ll += fn * std::log(p_fn);
            if (p_tp > 0.0) ll += tp * std::log(p_tp);
    ...
    static inline void constrained_fit_safe(
        ...
        if (!constrained_fit_quadratic(y, x, conf_mat, probas)) {
            ...
            const double n = static_cast<double>(
                conf_mat[0] + conf_mat[1] + conf_mat[2] + conf_mat[3]);
            const double metric_n = static_cast<double>(conf_mat[2] + conf_mat[3]);
...
#endif  // INCLUDE_MMU_CORE_MULTN_LOGLIKE_METRICS_HPP_
